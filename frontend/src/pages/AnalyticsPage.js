...
import { ArrowLeft, MessageSquare, Eye, TrendingUp, Calendar, Download, FileText, Zap, User, Crown, Lock } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0a0a0a]" data-testid="analytics-page">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {user?.subscription_tier === 'free' ? (
          <div className="relative overflow-hidden rounded-2xl border border-gray-800 bg-black/40 p-12 backdrop-blur-sm text-center mt-8">
            <div className="flex flex-col items-center justify-center z-10 relative">
              ...
              <h2 className="text-3xl font-bold text-white mb-3" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>Detailed Analytics Locked</h2>
              <p className="text-gray-400 mb-8 max-w-md mx-auto">
                Upgrade to the <strong className="text-emerald-400">Creator</strong> or <strong className="text-purple-400">Growth</strong> plan to unlock deep insights, interaction metrics, and AI recruiter tracking.
              </p>
              <Button onClick={() => navigate('/pricing')} className="bg-gradient-to-r from-emerald-500 to-lime-500 hover:from-emerald-600 hover:to-lime-600 text-black px-8 py-6 text-lg font-bold shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                <Crown className="w-5 h-5 mr-2" /> Upgrade Account
              </Button>
            </div>
          </div>
        ) : (
                      ...
                      <tr key={session.id} className="hover:bg-emerald-500/5 transition-colors group">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <Button
                            ...
                            className={`font-bold ${user?.subscription_tier === 'growth' ? 'text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10' : 'text-gray-500 hover:bg-transparent cursor-not-allowed'}`}
                            disabled={summarizing === session.id || user?.subscription_tier !== 'growth'}
                            title={user?.subscription_tier !== 'growth' ? "Requires Growth Plan" : "Export Recruiter Snapshot"}
                          >
                            {user?.subscription_tier !== 'growth' ? (
                              <><Lock className="w-4 h-4 mr-2" /> Requires Growth Plan</>
